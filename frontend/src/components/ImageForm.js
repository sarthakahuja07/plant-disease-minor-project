...
import { generateThunk } from "../redux/resultSlice";

const ImageForm = () => {
	const dispatch = useDispatch();
	const { file, loading } = useSelector((state) => state.fileState);
	const { isAuthenticated } = useSelector((state) => state.authState);
	const { isGenerated, result } = useSelector((state) => state.resultState);
		...
		<>
			<Box
				sx={{
					...
					padding: [0, 2],
					background:
						"linear-gradient(125.56deg, rgba(255, 255, 255, 0.35) 16.45%, rgba(255, 255, 255, 0.1) 70.19%)",
					borderRadius: 3,
					backdropFilter: "blur(10px)",
					boxShadow: "0px 4px 24px -1px rgba(0, 0, 0, 0.2)",
					backgroundImage:
						"linear-gradient(125.56deg, rgba(24, 26, 27, 0.45) 16.45%, rgba(24, 26, 27, 0.3) 70.19%)",
